var CalendarController = function($scope) {
	...
	$scope.showCalander=function (_flag) {
		if(_flag==0) {
	...
	function bindEvent() {
		...
		var _tds=_table.getElementsByTagName("td");
		for(var i=0; i<_tds.length; i++) {
			addEvent(_tds[i],'click',writeInput);
		}
	};
	...
	function writeInput(e) {
		if($scope.flag==0) {
			...
			_beginDate.attr("value",$(e.currentTarget).attr("title"));
			$("#calendar").css("display","none");			
		} else {
			var _endDate=$('#endDate');
			_endDate.attr("value",$(e.currentTarget).attr("title"));
			$("#calendar").css("display","none");
		}
